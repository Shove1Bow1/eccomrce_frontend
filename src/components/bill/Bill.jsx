...
const Bill = () => {
    ...
    useEffect(() => {
        function DirectToPayment() {
            if (localStorage.getItem("userId") && getCountItemCart() !== 0) {
                console.log("1")
                instance.post('/payments/create', { userId: localStorage.getItem("userId"), products: getAllItemQuantity(), totalPrice: sumPrice() }).then(res => res.data).then(data => { setClientSercet(data.data.clientSecret) }).catch(err => { if (err) successNotification(TYPE_NOTIFICATTION.ERROR, "Error", err) })
            }
        }
        if (!clientSercet) {
            DirectToPayment();
